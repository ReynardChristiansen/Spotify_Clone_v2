...
const PlayerContextProvider = (props) => {
  ...
  const [track, setTrack] = useState({
    id: "mhycwNJZ",
    image:
      "https://c.saavncdn.com/519/Being-Funny-In-A-Foreign-Language-English-2022-20221125000319-500x500.jpg",
    name: "About You",
    ...
    async function getArtist() {
      try {
        ...
        const newArtistId = final.data[0].artists.primary[0].id
          ? final.data[0].artists.primary[0].id + ""
          : "";
        ...
        if (newArtistId) {
          try {
            ...
            if (final_temp.data.topSongs.length == 0) {
              ...
              try {
                const apiRes = await fetch(
                  `https://hirmify-api.vercel.app/api/users/${ids}`,
                  {
                    method: "GET",
                    headers: {
                ...
                if (randomLikeSong.song_id == track.id) {
                  ...
                  const songName =
                    randomLikeSong.song_name.length > 15
                      ? randomLikeSong.song_name.slice(0, 15) + "..."
                      : randomLikeSong.song_name;
                  setName(songName);
              ...
              const randomIndex = Math.floor(
                Math.random() * final_temp.data.topSongs.length
              );
              ...
              if (track.id === randomSong.id) {
                ...
                try {
                  const apiRes = await fetch(
                    `https://hirmify-api.vercel.app/api/users/${id}`,
                    {
                      method: "GET",
                      headers: {
                  ...
                  const filteredSongs = tempss.filter(
                  ...
                  if (filteredSongs.length == tempss.length - 1) {
                    ...
                    if (randomSong.song_id == track.id) {
                      ...
                      const songName =
                        randomSong.song_name.length > 15
                          ? randomSong.song_name.slice(0, 15) + "..."
                    ...
                    const songName =
                      randomSong.name.length > 15
                        ? randomSong.name.slice(0, 15) + "..."
                        : randomSong.name;
                    setName(songName);
                    setId(randomSong.id);
                  }
  ...
  const previousSong = async () => {
    const currentIndex = playlistHistory.findIndex(
      (song) => song.id === track.id
    );
    const previousIndex =
      (currentIndex - 1 + playlistHistory.length) % playlistHistory.length;
  ...
  const like = async (song_url, song_image, song_name, song_id) => {
    ...
    try {
      const apiRes = await fetch(
        `https://hirmify-api.vercel.app/api/users/update/${id}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const deleteSong = async (song_id_user) => {
    ...
    try {
      const response = await fetch(
        `https://hirmify-api.vercel.app/api/users/delete/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
